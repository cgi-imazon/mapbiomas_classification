...
PATH_LOGFILE = f'{PATH_DIR}/data/log_amazon.csv'
...
roi = ee.FeatureCollection(ASSET_ROI).filter(ee.Filter.eq('Bioma', 'Amazônia'))
...
def get_dataset(tile_id: str):
    ...
    asset_feat_tile = '{}/feature-space-{}-9-{}'.format(ASSET_FEATURES, str(year), OUTPUT_VERSION)
    print(asset_feat_tile)
    # projects/imazon-simex/LULC/feature-space/feature-space-1985-9-1
    ...
    image = ee.Image(asset_feat_tile).clip(roi)
...
for year in YEARS:
    ...
    if not os.path.exists(f'{PATH_DIR}/data/{str(year)}'):
        os.mkdir(f'{PATH_DIR}/data/{str(year)}')
