...
@retry()
def classify_image(image_id: str):


    log = open(PATH_LOGFILE, 'a+')

    imagename = '{}_{}_{}'.format(image_id, str(year),OUTPUT_VERSION)
    ...
    try:
        ...
        if os.path.isfile(f'{PATH_DIR}/data/{str(year)}/{tile}/{image_id}.geojson'):
            df_target_sample = gpd.read_file(f'{PATH_DIR}/data/{str(year)}/{tile}/{image_id}.geojson')
...
def export_predictions(image_ids: list):

    future_to_point = {EXECUTOR.submit(classify_image, image_id): image_id for image_id in image_ids}
